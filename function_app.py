...
def BlobTrigger(myblob: func.InputStream):
    ...
    # Zápis dát do Application Insight - Azure Monitor
    tc = TelemetryClient(os.environ.get('APPINSIGHTS_INSTRUMENTATIONKEY'))

    # Odoslanie metrík do Application Insights
    tc.track_metric('Teplota', temperature)
    tc.track_metric('Vlhkosť', humidity)
    tc.track_metric('CO2', co2)
    tc.track_metric('IAQ', iaq)
    tc.track_metric('Tlak', pressure)
    tc.track_metric('Napätie batérie', battery_v)

    # Odoslanie dát do Azure Monitor
    tc.flush()
